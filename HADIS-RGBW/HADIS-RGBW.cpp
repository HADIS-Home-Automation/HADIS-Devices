...
void hostAP() {
    ...
    // turn on BLUE led during AP hosting
    ...
    WiFi.mode(WIFI_STA);
    WiFi.hostname("HADIS-RGBW-Controller");
...
void connectToMqtt() {
    ...
    if (pubSubClient.connect(clientID, topicStatus, 0, TRUE, "OFFLINE")) {
        ...
        // subscribe to topics
        pubSubClient.subscribe(topicRGBW);
...
void callback(char *topic, const byte *payload, unsigned int length) {
    ...
    // topic for RGBW brightness control
    if (!strcmp(topic, topicRGBW) && length == 9) {

        // remove of '#' and convert to int
        unsigned int numberRGBW = strtoll(reinterpret_cast<const char *>(payload + 1), nullptr, 16);

        // calculate pwm for RED led
        RED = numberRGBW >> 24;
        RED = RED << 2;

        // calculate pwm for GREEN led
        GREEN = numberRGBW >> 16 & 0xFF;
        GREEN = GREEN << 2;

        // calculate pwm for BLUE led
        BLUE = numberRGBW >> 8 & 0xFF;
        BLUE = BLUE << 2;

        // calculate pwm for WHITE led
        WHITE = numberRGBW & 0xFF;
        WHITE = WHITE << 2;


        // turn on pwm for RGBW
