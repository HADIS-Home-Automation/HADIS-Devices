...
float temp = 0;
float hum = 0;
unsigned int I2Cdata[6];
const int TEMP_HUM_READ_DELAY = 60000;

// I2C address for WEMOS is 0x45 from datasheet
...
char topicStatus[90];
char msg[50];
int MQTT_CONNECTION_FAILED_TIMES = 0;
int MQTT_CONNECTION_FAILED_TIMES_MAX = 5;
...
void generateTopics() {
    ...
    // build topic suffix
    strcpy(topicTemp, topicSetup);
    strcpy(topicHum, topicSetup);
    strcpy(topicStatus, topicSetup);
    strcat(topicSetup, "/SETUP");
    strcat(topicTemp, "/TEMP");
    strcat(topicHum, "/HUM");
    strcat(topicStatus, "/STATUS");
...
void connectToMqtt() {
    ...
    // if mqtt connection failed more than MQTT_CONNECTION_FAILED_TIMES_MAX
    // start hosting setup Access Point
    if (MQTT_CONNECTION_FAILED_TIMES >= MQTT_CONNECTION_FAILED_TIMES_MAX) {
        hostAP();

        // reset the counter
        MQTT_CONNECTION_FAILED_TIMES = RESET;
    ...
    if (pubSubClient.connect(clientID, topicStatus, 0, TRUE, "OFFLINE")) {
        ...
        // if connection failed increase failed times counter
        MQTT_CONNECTION_FAILED_TIMES++;
...
void I2CRead() {
    ...
    // send measurement command MSB (enabled clock stretching)
    Wire.write(0x2C);

    // send measurement command LSB (high repeatability)
    Wire.write(0x06);
    ...
    // convert read temp to temperature celsius with formula:
    // 175 * SensorTemp/(2^16-1) - 45
    // SensorTemp = temp MSB * 2^8 + temp LSB
    temp = 175 * (I2Cdata[0] * 256.0 + I2Cdata[1]) / 65535.0 - 45;

    // convert read humidity to relative humidity with formula:
    // 100 * SensorHum/(2^16-1)
    // SensorHum = humidity MSB * 2^8 + humidity LSB
    hum = 100 * (I2Cdata[3] * 256.0 + I2Cdata[4]) / 65535.0;
...
unsigned int previousTempHumReadTime = 0;
...
void loop() {
    ...
    // publish measurement every TEMP_HUM_READ_DELAY ms
    if (millis() - previousTempHumReadTime >= TEMP_HUM_READ_DELAY) {
        I2CRead();

        gcvt(temp, 6, msg);
        pubSubClient.publish(topicTemp, msg, TRUE);

        gcvt(hum, 6, msg);
        pubSubClient.publish(topicHum, msg, TRUE);

        previousTempHumReadTime = millis();
    }
