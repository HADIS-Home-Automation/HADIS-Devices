...
ICACHE_RAM_ATTR void isrButton() {
    ...
    if (interruptTime - lastInterruptTime > 200) {
        ...
        digitalWrite(LED_Button, relayState);
...
void hostAP() {
    // indicator LED is on during Access point hosting
...
void longPress() {
    ...
    // turn indicator LED ON during this time
    hostAP();
}
...
void connectToMqtt() {
    ...
    if (pubSubClient.connect(clientID, topicStatus, 0, TRUE, "OFFLINE")) {
        ...
        // on successful connection publish current relay state
        itoa(relayState, msg, 10);
        pubSubClient.publish(topicSwitch, msg, TRUE);

        // subscribe to topics
        pubSubClient.subscribe(topicSwitch);
...
void callback(char *topic, const byte *payload, unsigned int length) {
    ...
    // topic for relay state control
    if (!strcmp(topic, topicSwitch)) {

        // only change relay state if command not issued by this device
        if (!hardwareTrigger) {
...
void setup() {
    ...
    pinMode(LED_Button, FUNCTION_3);
    //GPIO 3 (RX) swap the pin to a GPIO.
    pinMode(pinButton, FUNCTION_3);
...
void loop() {
    ...
    // keep track of relay state changes
    if (lastRelayState != relayState) {

        // publish relay state to MQTT if triggered by hardware
        if (!mqttTrigger) {
            // convert relay state to char array & publish
            itoa(relayState, msg, 10);
            pubSubClient.publish(topicSwitch, msg, TRUE);
        }
        mqttTrigger = FALSE;

        // set last relay state for detecting changes
        lastRelayState = relayState;
    }
}
