...
char topicStatus[90];
char msg[50];
int mqttTrigger1 = FALSE;
int mqttTrigger2 = FALSE;
...
// ISR routine for device button1 press
// Setup mode long press only started on button1
ICACHE_RAM_ATTR void isrButton1() {
...
// ISR routine for device button2 press
ICACHE_RAM_ATTR void isrButton2() {
    static unsigned long lastInterruptTime = 0;
    unsigned long interruptTime = millis();
...
void longPressPoll() {
    ...
    if (buttonPressedTime > 0) {
        ...
        // if button1 is pressed for LONG_PRESS_LENGTH start long press function
        if (millis() - buttonPressedTime >= LONG_PRESS_LENGTH) {
            buttonPressedTime = RESET;
            longPress();
        }
            // stop the count if button1 is not pressed anymore
        else if (digitalRead(pinButton1)) {
            PRINTLN("RESET");
            buttonPressedTime = RESET;
        }
    }
...
void setup() {
    ...
    attachInterrupt(digitalPinToInterrupt(pinButton1), isrButton1, FALLING);
    attachInterrupt(digitalPinToInterrupt(pinButton2), isrButton2, FALLING);
...
void loop() {
    ...
    // keep track of RELAY1 changes
    if (lastRelay1State != relay1State) {
        ...
        // publish RELAY1 state to MQTT if triggered by hardware
        if (!mqttTrigger1) {
        ...
        // set last RELAY1 state for detecting changes
        lastRelay1State = relay1State;
    }

    // keep track of RELAY2 changes
    if (lastRelay2State != relay2State) {
        ...
        // publish RELAY2 state to MQTT if triggered by hardware
        if (!mqttTrigger2) {
        ...
        // set RELAY2 state for detecting changes
        lastRelay2State = relay2State;
    }
}
